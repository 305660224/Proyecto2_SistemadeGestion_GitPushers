 ...
 * @author danny
 */
public class UtilGuis {
    private static boolean hasValue(JComponent txt) {
        switch (txt) {
            case JTextComponent text -> {
                String s = text.getText();
                return s != null && !s.trim().isEmpty();// trim quita los espacios y se esta viendo si se quitan los espacios queda vacio
            }
            case JComboBox<?> combo -> {
                Object i = combo.isEditable() ? combo.getEditor().getItem() : combo.getSelectedItem();// ? = Operador ternario (Es como un if comprimido) : = Sino
